...
按字幕重新对标，补全 PPT 第 13 页讲的核心概念：

### 🌟 新增

- **数据三分**：练习池 70% / 模拟考池 15% / 高考池 15%（替代 v3 的二分）
- **🎓 高考成绩只能公布一次**：战绩页 Tab 切到高考，需点"📢 公布"按钮，按完锁死
- **🔍 自动网格搜索**：渔具店顶部按钮，跑 12 组装备组合，按模拟考损失排名，点"采用"自动填入
- **👤 vs 🤖 分工标注**：进度条、shop-card 头部标明哪步是人决定哪步是机器跑
- **🔍 泛化能力诊断**：高考公布后自动对比"模拟考 vs 高考"损失，识别"过拟合到模拟考池"

### 📊 数据池可视化

钓鱼场 Canvas 上三种鱼一目了然：
- 🟧 训练鱼（彩色）：每 epoch 都参与梯度下降
- 🟪 验证鱼（紫 + 标签 V）：早停判断 / 实时显示模拟考损失
- 🔴 测试鱼（红 + ?锁）：训练时被锁死成灰雾的 "?"，公布后才显示真实位置
